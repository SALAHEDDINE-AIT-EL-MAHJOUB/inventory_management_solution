...
const CreateClientForm = ({ adminUserId, onClientCreated }) => {
  ...
  const testConnection = async () => {
    try {
      const response = await axios.post("/api/Test/simple", { test: "connection" });
      console.log("Test de connexion réussi:", response.data);
      setMessage("Connexion au serveur OK");
    } catch (err) {
  ...
  const testClientData = async () => {
    try {
      ...
      const response = await axios.post("/api/Test/client-test", testData);
      console.log("Test données client réussi:", response.data);
      setMessage("Test données client OK");
    } catch (err) {
      console.error("Test données client échoué:", err);
      setError("Problème avec les données client");
    }
  };
    ...
    <div className="create-client-container">
      <form className="create-client-form" onSubmit={handleSubmit}>
        ...
        <div style={{ marginBottom: '20px', textAlign: 'center' }}>
          ...
          <button type="button" onClick={testClientData}>
            Test Données
          </button>
        </div>
